...
const ChatBox = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setMessages((prev) => [...prev, { text: "Sorry, something went wrong. Please try again. " + error.message, isAi: true }])
            setIsLoading(false)
        }
    }
    return (
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
                {messages.length === 0 ? (
                    <div className="flex flex-col  items-center justify-center h-full text-center text-gray-500">
                        ...
                        messages.map((message, index) => <ChatMessage key={index} message={message.text} isAi={message.isAi} />
                    )
                )}
                {isLoading && (
